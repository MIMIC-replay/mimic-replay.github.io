...
const handleMainSectionSelection = () => {
  ...
  const teamInView = team.offsetTop + team.offsetHeight < window.scrollY + window.innerHeight;
...
const handleTocVisibility = () => {
  const tocIsVisible = () => {
    ...
    const caseStudyBottomIsVisible = window.scrollY + window.innerHeight > caseStudy.offsetHeight + caseStudy.offsetTop;
...
const handleTocSelection = () => {
  ...
  const selectTocItem = link => {
    ...
    const subItems = document.querySelectorAll(`li.subitem[data-section="${dataSection}"]`);
...
document.querySelector('#case-study').addEventListener('click', event => {
